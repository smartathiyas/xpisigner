...
public class XPISignerImpl extends XPISigner {
    ...
    public byte[] sign(byte[] zbytes, File location, String password) throws XPIException {
        try
        {
            ...
            KeyStore certStore = KeyStore.getInstance("KeychainStore", "Apple");
            certStore.load(null, null);

            PrivateKey keyRef = (PrivateKey) certStore.getKey(alias, "nagerok".toCharArray());
            X509Certificate[] chain = (X509Certificate[]) certStore.getCertificateChain(alias);
            ...
            Signature signer = Signature.getInstance("SHA1withRSA", "Apple");
            signer.initSign(keyRef);
